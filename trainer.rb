...
if run_percentages
  # need to figure out ticket scheme
  headers = ["survived", "pclass", "last_name", "sex", "age", "sibsp", "parch", "fare", "embarked", 'cabin_number', 'cabin_letter', 'normalized_fare', 'normalized_age'].map(&:intern)
...
if run_combos
  combos = headers.combination(2).to_a

  combos.each do |combo|
    puts "by " << combo.join(", ")
    values = passengers.map{|x| x.gimme(combo)}.uniq
    values.each do |val|

      cow = passengers.select{|x| x.send(:gimme, combo) == val}
      live, dead = cow.partition{|x| x.survived?}
      unless cow.count < 10

        # g = Gruff::Scatter.new(800)
        # g.title = combo.zip(val).join(", ")
        # g.data(:control, [0, 10], [0, 10])
        # live_xs = live.map{|x| x.send(:gimme, combo)[0].to_i}
        # puts live_xs.inspect
        # live_ys = live.map{|x| x.send(:gimme, combo)[1].to_i}
        # g.data(:live, live_xs, live_ys)
        # dead_xs = dead.map{|x| x.send(:gimme, combo)[0].to_i}
        # dead_ys = dead.map{|x| x.send(:gimme, combo)[1].to_i}
        ...
        if percentage > 0.7 || percentage < 0.3
          puts "\t\t" << cow.count.to_s <<  " people had #{val} for #{combo}"
